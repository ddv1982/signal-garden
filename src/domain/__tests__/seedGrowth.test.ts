...
import { advanceGardenGrowth, assignPlotToSeed, bloomSeed, growthStageForSeed, waterSeed } from '../seedGrowth';
...
describe('seed growth domain', () => {
  it('assigns a chosen garden plot with local growth metadata', () => {
    const planted = assignPlotToSeed(seed, 'front-left', { x: 0.2, y: 0.8 }, '2026-06-07T10:00:00.000Z');
  ...
  it('waters a sprouted seed into a growing bud without blooming', () => {
    const planted = assignPlotToSeed(seed, 'front-left', { x: 0.2, y: 0.8 }, '2026-06-07T10:00:00.000Z');
    const sprouted = waterSeed(planted, { transformedLabel: 'A passing story.', kindAction: 'Breathe.' }, '2026-06-07T10:03:00.000Z');
    const growing = waterSeed(sprouted, { transformedLabel: 'A smaller story.', kindAction: 'Step outside.' }, '2026-06-07T10:06:00.000Z');
  ...
  it('blooms a growing seed into a flower after final reflection', () => {
    ...
    const sprouted = waterSeed(planted, { transformedLabel: 'A passing story.', kindAction: 'Breathe.' }, '2026-06-07T10:03:00.000Z');
    const growing = waterSeed(sprouted, { transformedLabel: 'A smaller story.', kindAction: 'Step outside.' }, '2026-06-07T10:06:00.000Z');
    const flower = bloomSeed(growing, { outcome: 'adapted', reflection: 'The action became a smaller kinder step.' }, '2026-06-07T10:09:00.000Z');
  ...
  it('rejects bloom reflection before two waterings', () => {
    ...
    expect(() => bloomSeed(planted, { outcome: 'done', reflection: 'It became a flower.' }, '2026-06-07T10:06:00.000Z')).toThrow(
      'Seed bloom requires two waterings before final reflection.'
    );
    expect(() => bloomSeed(sprouted, { outcome: 'done', reflection: 'It became a flower.' }, '2026-06-07T10:06:00.000Z')).toThrow(
      'Seed bloom requires two waterings before final reflection.'
    );
  });
  ...
  it('keeps dream-heavy seeds botanical instead of becoming lanterns', () => {
    const dreamSeed = assignPlotToSeed(
      { ...seed, dreams: ['a small gray cloud'], values: ['rest'], bodySignals: ['tight chest'], emotions: ['sad'] },
      'front-left',
      { x: 0.2, y: 0.8 },
      '2026-06-07T10:00:00.000Z'
    );
    const sprouted = waterSeed(dreamSeed, { transformedLabel: 'A passing story.', kindAction: 'Breathe.' }, '2026-06-07T10:03:00.000Z');
    const growing = waterSeed(sprouted, { transformedLabel: 'A smaller story.', kindAction: 'Step outside.' }, '2026-06-07T10:06:00.000Z');
    const flower = bloomSeed(growing, { outcome: 'done', reflection: 'This became a flower.' }, '2026-06-07T10:09:00.000Z');
  ...
  it('records water on a blooming seed without growing beyond bloom', () => {
    ...
    const watered = waterSeed(blooming, { transformedLabel: 'Already softer.', kindAction: 'Keep resting.' }, '2026-06-07T10:09:00.000Z');
  ...
  it('rejects blank watering text without advancing growth', () => {
    ...
    expect(() => waterSeed(planted, { transformedLabel: '   ', kindAction: 'Breathe.' }, '2026-06-07T10:03:00.000Z')).toThrow(
      'Seed watering requires a softened label and kind action.'
    );
    expect(() => waterSeed(planted, { transformedLabel: 'A passing story.', kindAction: '   ' }, '2026-06-07T10:03:00.000Z')).toThrow(
      'Seed watering requires a softened label and kind action.'
    );
